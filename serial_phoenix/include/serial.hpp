...
/**
 * @brief 串口操作返回码, 且可以作为 bool 进行逻辑运算
 * @note 用于表示串口操作的结果状态
 */
struct SerialCode {
    // 串口操作返回码
    enum class Value : int {
        ...
        OPEN_NO_CALLBACK   = 13, // 自动查找时未提供回调函数
    ...
    constexpr bool operator&&(const SerialCode& rhs) const noexcept {
...
public:
     ...
     * @param config 串口配置
     * @param bytes_ 每包字节数
     * @param find_callback 自动查找回调函数
     * @return SerialCode 串口操作返回码
     * @note 优先调用 find_callback 来获取串口地址, 此时会覆盖 port 的地址, 且不会回退至 port 查找 
    */
    SerialCode open(
        std::string port                           = "/dev/ttyACM0",
        std::shared_ptr<SPconfig> config           = nullptr,
        size_t bytes_                              = 32,
        std::function<std::string()> find_callback = nullptr
    );
     ...
     * @brief 读取数据
     * @note 该函数是线程安全的，会加锁
     * @note 避免与不加锁的 read_unsafe() 函数混用
     * 
     * @tparam T 输出数据类型, 由于使用 memcpy 进行数据拷贝, T 需要是标准布局类型, 或是 std::vector<uint8_t>
     * @param out 输出数据引用
     * @return SerialCode 串口操作返回码
     */
    template<typename T>
    ...
    /**
     * @brief 读取数据
     * @note 该函数不是线程安全的，对串口读取不加锁, 且使用了未加锁的缓冲区
     * @note 用户需要自行确保线程安全
     * 
     * @tparam T 输出数据类型, 由于使用 memcpy 进行数据拷贝, T 需要是标准布局类型, 或是 std::vector<uint8_t>
     * @param out 输出数据引用
     ...
     * @brief 写入数据
     * @note 该函数是线程安全的，会加锁
     * @note 避免与不加锁的 write_unsafe() 函数混用
     * 
     * @tparam T 输入数据类型, 由于使用 memcpy 进行数据拷贝, T 需要是标准布局类型, 或是 std::vector<uint8_t>
     * @param in 输入数据引用
     * @return SerialCode 串口操作返回码
     */
    template<typename T>
