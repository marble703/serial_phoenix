...
SerialCode Serial::open(
    std::string port,
    std::shared_ptr<SPconfig> config,
    size_t bytes_,
    std::function<std::string()> find_callback
) {
    if (this->is_open()) {
    ...
    if (std::filesystem::exists(port) == false) {
        std::cerr << "[Serial::open] Error: Serial port " << port << " does not exist."
                  << std::endl;
...
SerialCode Serial::close() {
    // 对于 lock_guard 无法使用 try_lock
    // if (this->read_mutex_.try_lock() == false) {
    //     return SerialCode::Value::CLOSE_READING;
    // }
    // this->read_mutex_.unlock();

    // if (this->write_mutex_.try_lock() == false) {
    //     return SerialCode::Value::CLOSE_WRITING;
    // }
